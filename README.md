...
This is application-layer detection, not a systemic fix. The real mitigation needs to happen in extraction libraries: openpyxl, pandas, and markitdown should offer an option to return formatted display values, or at minimum surface the format string alongside the raw value so downstream consumers can detect divergence. The underlying issues are open upstream ([pandas #30272](https://github.com/pandas-dev/pandas/issues/30272), [pandas #61539](https://github.com/pandas-dev/pandas/issues/61539)). Until they're patched, any pipeline that ingests documents for LLM analysis should run a divergence check before passing data to the model.
